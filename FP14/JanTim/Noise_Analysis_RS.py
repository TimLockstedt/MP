...
import os
import sys
from   matplotlib.mlab import normpdf, Rectangle
...
def main(fname,options):
    ...
    times = pd.DataFrame((hdf.File(fname)['/events/time']).value)
    pedestalF = (pd.DataFrame((hdf.File(fname)['/header/pedestal']).value))
    noiseF = (pd.DataFrame((hdf.File(fname)['/header/noise']).value))
    signal_raw = pd.DataFrame((hdf.File(fname)['/events/signal']).value)
